...
{
    public UnityEvent HeadBobed;
    private Image blackScreen;
    private Rigidbody rb;
    private float _currentHeadBobSin;
    private float _headBobSin;
    private bool _headBobed;
    ...
    private void HeadBob()
    {
        ...
        if(isWalking)
        {
            ...
            _currentHeadBobSin = Mathf.Sin(timer);

            joint.localPosition = new Vector3(jointOriginalPos.x + _currentHeadBobSin * bobAmount.x, jointOriginalPos.y + _currentHeadBobSin * bobAmount.y, jointOriginalPos.z + _currentHeadBobSin * bobAmount.z);

            if (_currentHeadBobSin > _headBobSin && _headBobed == false)
            {
                HeadBobed?.Invoke();
                _headBobed = true;
            }

            if (_currentHeadBobSin < _headBobSin)
                _headBobed = false;
            
            _headBobSin = _currentHeadBobSin;
